...
describe("WellnessController", () => {
  ...
  beforeEach(() => {
    wellnessService = {
      getFormsPage: jest.fn(),
      getFormById: jest.fn(),
      submitForm: jest.fn(),
      updateForm: jest.fn(),
      deleteForm: jest.fn(),
      getRecentForms: jest.fn(),
    };
  ...
  it("getAllForms passes memberId from query", async () => {
    ...
    await controller.getAllForms({ query: { memberId, limit: 20 } }, { status, locals: { requestId: "t" } }, jest.fn());
  ...
  it("getFormById delegates", async () => {
    ...
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    await controller.getFormById({ params: { id } }, { status }, jest.fn());
  ...
  it("submitForm returns 201", async () => {
    ...
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    await controller.submitForm({ body }, { status }, jest.fn());
  ...
  it("updateForm delegates", async () => {
    ...
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    await controller.updateForm({ params: { id }, body: { fatigue: 6 } }, { status }, jest.fn());
  ...
  it("deleteForm returns 204", async () => {
    ...
    await controller.deleteForm({ params: { id } }, { status }, jest.fn());
  ...
  it("getRecentForms passes memberId and days", async () => {
    ...
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    await controller.getRecentForms({ params: { memberId }, query: { days: "7" } }, { status }, jest.fn());
